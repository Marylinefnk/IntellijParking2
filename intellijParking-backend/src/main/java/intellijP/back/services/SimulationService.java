...
public class SimulationService {
    ...
    public InitialisationResultatDTO initialiser(int nbPersonnes, int maxVehiculesParPersonne, Long seed) {
        ...
        for (Place place : toutesLesPlaces) {
            if (!capteurRepository.existsByPlaceId(place.getId())) {
                capteurRepository.save(Capteur.builder().place(place).etatCapteur(EtatCapteur.ACTIF).presenceDetectee(false).dateDernierSignal(null).vehiculeDetecte(null).build()); compteurCapteurs++;
            }
        }
    ...
    public void genererReservationsJournee(LocalDate date, int pasSecondes, String niveau, Double tauxCible) {
        ...
        if (places.isEmpty()) { logger.warn("Aucune place trouvée - niveau={}", niveau); return; }
        List<Vehicule> tousVehicules = vehiculeRepository.findAll();
        if (tousVehicules.isEmpty()) { logger.warn("Aucun vehicule en base"); return; }
        ...
        for (Place place : places) {
            ...
            while (curseur.isBefore(finJournee)) {
                ...
                try { Thread.sleep(pasSecondes*1000L); } catch (InterruptedException e) { Thread.currentThread().interrupt(); logger.warn("Génération interrompue"); return; }
            }
        }
    ...
    public void demarrerSimulation(int intervalleSecondes, String niveau, double probaPresence) {
        if (simulationActive) { logger.warn("Simulation déjà active, on ignore la demande"); return; }
        ...
        while (simulationActive) {
            try {
                ...
                if (capteurs.isEmpty()) { logger.warn("Aucun capteur actif trouvé - simulation en attente"); }
                else {
                    Capteur capteur = capteurs.get(rng.nextInt(capteurs.size()));
                    boolean nouvellePresence = rng.nextDouble() < probaPresence;
                    if (nouvellePresence != capteur.isPresenceDetectee()) traiterChangementPresence(capteur, nouvellePresence);
                }
    ...
    public void arreterSimulation() { simulationActive = false; logger.info("Arrêt simulation demandé"); }
    ...
    public void traiterChangementPresence(Capteur capteur, boolean nouvellePresence) {
        ...
        if (nouvellePresence) {
            ...
            for (ReservationPlace r : resasActives) {
                if (r.getVehicule().getId().equals(vehiculeTrouve.getId())) { resa = r; typeResa = "PREEXISTANTE"; break; }
            }
            if (resa != null) {
                statutResaAvant = resa.getStatut().name(); resa.setStatut(StatutReservation.EN_COURS); reservationPlaceRepository.save(resa);
            } else {
                resa = ReservationPlace.builder().place(place).vehicule(vehiculeTrouve).personne(vehiculeTrouve.getPersonne())
                        .dateDebut(LocalDateTime.now()).dateFin(LocalDateTime.now().plusHours(2)).statut(StatutReservation.EN_COURS).build();
                reservationPlaceRepository.save(resa);
            }
            stationnementRepository.save(Stationnement.builder().place(place).vehicule(vehiculeTrouve).dateEntree(LocalDateTime.now()).build());
            capteur.setPresenceDetectee(true); capteur.setVehiculeDetecte(vehiculeTrouve); capteur.setDateDernierSignal(LocalDateTime.now()); capteurRepository.save(capteur);
            evenementCapteurRepository.save(EvenementCapteur.builder().capteur(capteur).place(place).vehicule(vehiculeTrouve).reservation(resa).presenceDetectee(true).dateEvenement(LocalDateTime.now()).source(SourceEvenement.SIMULATION).build());
            StatutPlace ancienStatut = place.getStatut(); place.setStatut(StatutPlace.OCCUPEE); placeRepository.save(place);
            fluxSSEService.diffuserChangementPlace(construireDTO(place, ancienStatut, StatutPlace.OCCUPEE, "CAPTEUR", vehiculeTrouve, resa, typeResa, statutResaAvant, "EN_COURS"));
        } else {
            Vehicule vehiculePartant = capteur.getVehiculeDetecte();
            if (vehiculePartant == null) { logger.warn("Départ détecté mais pas de vehicule connu sur place {}", place.getNumero()); return; }
            ...
            if (statActif.isPresent()) {
                Stationnement s = statActif.get(); s.setDateSortie(LocalDateTime.now());
                long dureeMin = java.time.temporal.ChronoUnit.MINUTES.between(s.getDateEntree(), s.getDateSortie());
                s.setDureeMin((int)dureeMin); s.setTarif(Math.round((dureeMin/60.0)*2.0*100.0)/100.0); stationnementRepository.save(s);
            }
            ReservationPlace resaActive = null;
            List<ReservationPlace> resasEnCours = reservationPlaceRepository.findActiveReservationsAtMoment(place.getId(), LocalDateTime.now());
            for (ReservationPlace r : resasEnCours) { if (r.getStatut()==StatutReservation.EN_COURS) { resaActive = r; break; } }
            if (resaActive != null) { resaActive.setStatut(StatutReservation.TERMINEE); reservationPlaceRepository.save(resaActive); }
            capteur.setPresenceDetectee(false); capteur.setVehiculeDetecte(null); capteur.setDateDernierSignal(LocalDateTime.now()); capteurRepository.save(capteur);
            evenementCapteurRepository.save(EvenementCapteur.builder().capteur(capteur).place(place).vehicule(vehiculePartant).reservation(resaActive).presenceDetectee(false).dateEvenement(LocalDateTime.now()).source(SourceEvenement.SIMULATION).build());
            StatutPlace ancienStatut = place.getStatut(); StatutPlace nouveauStatut = StatutPlace.LIBRE;
            List<ReservationPlace> resasSuivantes = reservationPlaceRepository.findActiveReservationsAtMoment(place.getId(), LocalDateTime.now().plusSeconds(1));
            for (ReservationPlace r : resasSuivantes) { if (r.getStatut()==StatutReservation.CONFIRMEE) { nouveauStatut = StatutPlace.RESERVEE; break; } }
            place.setStatut(nouveauStatut); placeRepository.save(place);
            fluxSSEService.diffuserChangementPlace(construireDTO(place, ancienStatut, nouveauStatut, "CAPTEUR", vehiculePartant, resaActive, resaActive!=null?"PREEXISTANTE":null, "EN_COURS", resaActive!=null?"TERMINEE":null));
        }
    }
    ...
    private Vehicule trouverOuChoisirVehicule(Place place) {
        List<ReservationPlace> resasActives = reservationPlaceRepository.findActiveReservationsAtMoment(place.getId(), LocalDateTime.now());
        for (ReservationPlace r : resasActives) { if (r.getStatut()==StatutReservation.CONFIRMEE) return r.getVehicule(); }
        List<Vehicule> tousVehicules = vehiculeRepository.findAll(); Collections.shuffle(tousVehicules);
        for (Vehicule v : tousVehicules) { if (stationnementRepository.findActiveStationnementByVehicule(v.getId()).isEmpty()) return v; }
        return null;
    }
    ...
    private ChangementPlaceSSEDTO construireDTO(Place place, StatutPlace ancienStatut, StatutPlace nouveauStatut,
        ...
        dto.setDateEvenement(LocalDateTime.now()); dto.setIdPlace(place.getId()); dto.setNumero(place.getNumero());
        dto.setNiveau(place.getZone()!=null?place.getZone().getNom():null);
        dto.setAncienStatut(ancienStatut!=null?ancienStatut.name():null); dto.setNouveauStatut(nouveauStatut.name()); dto.setCause(cause);
        if (vehicule!=null) { dto.setIdVehicule(vehicule.getId()); dto.setImmatriculation(vehicule.getImmatriculation()); dto.setTypeVehicule(vehicule.getTypeVehicule()!=null?vehicule.getTypeVehicule().name():null); }
        if (resa!=null) { dto.setIdReservation(resa.getId()); dto.setTypeReservation(typeResa); dto.setStatutResaAvant(statutResaAvant); dto.setStatutResaApres(statutResaApres); }
        return dto;
    }
